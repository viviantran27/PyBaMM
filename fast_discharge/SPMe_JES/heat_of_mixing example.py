...
models = [
    pybamm.lithium_ion.SPMe(
        {"heat of mixing": "false", "thermal": "lumped"}, name="without heat of mixing"
    ),
    pybamm.lithium_ion.SPMe({"heat of mixing": "true","thermal": "lumped"}, name="with heat of mixing"),
]
...
for m, model in enumerate(models):
    sim = pybamm.Simulation(
        model, parameter_values=parameter_values, C_rate=C_rate, var_pts=var_pts
    )
    sim.solve(t_eval)
    sims.append(sim)
    ...
    axs[0, m].set_title(f"{model.name}" + "($Q_{mix} = j(U(c_{s,avg})-U(c_{s,surf}))$)" )
