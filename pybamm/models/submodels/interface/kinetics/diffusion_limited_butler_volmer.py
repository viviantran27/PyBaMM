...
class DiffusionLimitedButlerVolmer(BaseKinetics):
    """
    ...
    **Extends:** :class:`pybamm.interface.kinetics.BaseKinetics`
    ...
    def _get_kinetics(self, j0, ne, eta_r, T, variables):
        ...
        c_s_lim = 1e-4  #[mol.m-3] 1e-4 constant from paper

        if self.domain == "Positive": #Eqn 12

            c_s_p = variables["Positive particle concentration [mol.m-3]"]
            c_s_surf_p = pybamm.surf(c_s_p)
            c_p_max = self.param.c_p_max
            delta_c_s = c_p_max - c_s_surf_p

            c_e_p = variables["Positive electrolyte concentration [mol.m-3]"]
            c_e_surf_p = pybamm.surf(c_s_p) #pybamm.surf(c_e_p) what is this supposed to be?...
            delta_c_e = c_e_p - c_e_surf_p
            k2 = 1 #c_e_lim/pybamm.AbsoluteValue(delta_c_e) +  c_s_lim/pybamm.AbsoluteValue(delta_c_s)

        elif self.domain == "Negative": #Eqn 13
            c_s_n = variables["Negative particle concentration [mol.m-3]"]
            c_s_surf_n = pybamm.surf(c_s_n)
            c_n_max = self.param.c_n_max
            delta_c_s = c_n_max - c_s_surf_n
            k2 = c_s_lim/pybamm.AbsoluteValue(delta_c_s)

        return 2 * j0 * pybamm.sinh(k1 * eta_r)/ (1 + k2 * pybamm.exp(k1 * eta_r)) # + or - k1 in the denominator?
