    ...
    **Extends:** :class:`pybamm.thermal.BaseThermal`
    ...
    def set_rhs(self, variables):
        ...
        self.rhs = {
            ...
            # T_mid: (
            #     self.param.B * (Q_vol_av+ Q_decomp_mid)*gamma_mid - lambda_k*gamma_core * (T_core - T_mid)
            # )
            ...
            T_outer: (
                self.param.B * (Q_vol_av + Q_decomp_outer) - total_cooling_coefficient * (T_outer-T_amb) * gamma_outer + lambda_k*(gamma_mid + gamma_core) *(T_mid -T_outer)
            )
            / (self.param.C_th * self.param.rho(T_outer)),
            T_mid: (
                self.param.B * (Q_vol_av + Q_decomp_mid) - total_cooling_coefficient * (T_mid-T_amb) * gamma_mid + lambda_k*gamma_core * (T_core - T_mid) - lambda_k*(gamma_mid + gamma_core) *(T_mid -T_outer)
            )
            / (self.param.C_th * self.param.rho(T_mid)),
            T_core: (
                self.param.B * (Q_vol_av + Q_decomp_core + Q_isc) - total_cooling_coefficient * (T_core-T_amb) * gamma_core - lambda_k*gamma_core * (T_core - T_mid)
            )
