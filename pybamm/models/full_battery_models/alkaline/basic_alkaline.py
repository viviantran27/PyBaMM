#
# Basic alkaline model
#
import pybamm


class BasicFull(pybamm.alkaline.BaseModel):
    """Porous electrode model for alkaline, (from _____).

    This class differs from the :class:`pybamm.alkaline.Full` model class in that it
    ...
    **Extends:** :class:`pybamm.alkaline.BaseModel`
    """ 
...
# Main function that actually runs the code and simulates the model defined above
if __name__ == "__main__":
    import numpy as np

    basic_full = BasicFull() 
    # Solve basic Full mode
    sim = pybamm.Simulation(basic_full, solver=pybamm.CasadiSolver()) #sets default parameter values? 
    t_eval = np.linspace(0, 0.8)
    sim.solve(t_eval)
    sim.plot(
        [
            "Electrolyte concentration",
