    ...
    def set_up_experiment(self, model, experiment):
        ...
        for op, events in zip(experiment.operating_conditions, experiment.events):
            ...
            if op["dc_data"] is not None:
                ...
                if op_units == "Ohm":
                    operating_inputs.update(
                        {"Resistance switch": 1, "Resistance input [Ohm]": drive_cycle_interpolant}
                    )
            else:
                if op_units in ["A", "C"]:
                    ...
                    # Update inputs for constant resistance
                    R = op["electric"][0]
                    operating_inputs.update({"Resistance switch": 1, "Resistance input [Ohm]": R})               
        ...
        for op_cond, op_inputs in zip(
            ...
            if op_cond["electric"] not in self.op_conds_to_model_and_param:
                if op_inputs.get("Current switch") == 1:
                    ...
                    elif op_inputs.get("Resistance switch") == 1:
                        new_model.algebraic[
                            i_cell
                        ] = pybamm.external_circuit.ResistanceFunctionControl(
                            new_model.param, new_model.options, control="algebraic"
                        ).constant_resistance(
                            new_model.variables
                        )
                ...
                if "Voltage cut-off [V]" in op_inputs:
                    ...
                    if op_inputs.get("Power switch") == 1:
                        ...
                        inp = op_inputs["Resistance input [Ohm]"]
                    else:
                ...
                if op_inputs.get("Current switch") == 1:
                    ...
                    new_parameter_values.update(
                        {"Resistance function [Ohm]": op_inputs["Resistance input [Ohm]"]},
                        check_already_exists=False,
                    )
                elif op_inputs.get("CCCV switch") == 1:
                    new_parameter_values.update(
