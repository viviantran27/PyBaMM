    ...
    def read_string(self, cond, drive_cycles):
        ...
        if "Run" in cond:
            ...
            if all(x not in cond for x in dc_types):
                raise ValueError(
                    "Type of drive cycle must be specified using '(A)', '(V)', '(W)', or '(Ohm)'."
            ...
            # Check for Events
                ...
                # Find the type of drive cycle ("A", "V", "W", or "Ohm")
                typ = cond_list[2][1]
    ...
    def convert_electric(self, electric):
        ...
        if electric[0].lower() == "rest":
            ...
            if len(electric) in [3, 4]:
                if len(electric) == 4:
                    # e.g. Charge at 4 A, Hold at 3 V, Discharge at 0.05 Ohm
            ...
            if unit == "C":
                ...
                return (float(value), "Ohm")
            elif unit == "mOhm":
                return (float(value)/1000, "Ohm")
            else:
                raise ValueError(
                    """units must be 'C', 'A', 'mA', 'V', 'W', 'mW', 'Ohm', or 'mOhm', not '{}'.
                    For example: {}
                    """.format(
                        unit, examples
                    )
