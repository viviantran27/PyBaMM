    ...
    parameter_values.update(
        {
            ...
            "Typical current [A]":2.5,#,1C current
            "Current function [A]":2.5,#default current function,
...
def cycle_adaptive_simulation(model, parameter_values, experiment,SOC_0=1, save_at_cycles=None,drive_cycle=None):
    ...
    def y_to_sol(y, esoh_sim, model):
        ...
        for var in model.initial_conditions:
            ...
            elif var.name == "Charge capacity [A.h]":
...
def calculate_eSOH(C_n_init, C_p_init, x_init, y_init,model, parameter_values):    
    param = model.param
...
def plot1(all_sumvars_dict,esoh_data):
    ...
    esoh_data["Capacity [A.h]"]=esoh_data["Charge capacity [A.h]"]
...
def load_data_calendar(cell,eSOH_DIR,oCV_DIR):
    ...
    dfe['N']=dfe['Time']
    dfe["Capacity [A.h]"]=dfe["Charge capacity [A.h]"]
...
def init_exp_calendar(cell_no,dfe,param,parameter_values):
    # dfe_0 = dfe[dfe['N']==N[0]]
    C_n_init = dfe['Cn'].iloc[0]
    C_p_init = dfe['Cp'].iloc[0]
    y_0_init = dfe['y0'].iloc[0] 
...
#     dfe_0["Capacity [A.h]"]=dfe_0["Charge capacity [A.h]"]
